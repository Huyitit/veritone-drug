-- Check and recreate the events as they might have been removed in some environments
DO $$
DECLARE
  V_CREATED_BY CONSTANT VARCHAR := 'flyway';
...
-- Update the schedule to run every hour instead of once daily (if it exists)
UPDATE event_trigger.event_schedule
SET schedule = '0 * * * *',  -- At minute 0 of every hour
    updated_by = V_CREATED_BY,
    updated_at_utc = now()
WHERE event_name = EVENT_BACKFILL_SDO_RBAC;
