...
async function withClient<T>(database: string, fn: (client: Client) => Promise<T>): Promise<T> {
 ...
 */
export async function getJobContentApplicationId(jobId: string): Promise<string> {
 ...
 */
export async function getLlmGatewayLogBuffer(chatId: string): Promise<any> {
 ...
 */
export async function clearOpenIdExternalCredentialId(connectId: string): Promise<void> {
 ...
 */
export async function getOpenIdExternalCredentialId(connectId: string): Promise<string> {
  return withClient('sso', async (client) => {
    const res = await client.query(
