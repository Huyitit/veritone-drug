...
async function postRetry(
  ...
  return postRetry(url, data, options, --retryCount, isPatch);
}

async function signin(authUrl: string, options: { [key: string]: any } = {}) {
...
function responseStatusParserAllowAllStatus(respObj: { [key: string]: any }) {}

function responseParserThrowErrorOnBodyError(respObj: { [key: string]: any }) {
...
function responseParserThrowErrorOnNoBody(respObj: { [key: string]: any }) {
...
function responseParser(respObj: { [key: string]: any }) {
  return respObj;
}
 ...
 * ]
 */
function getErrorsFromGraphqlResponse(err: any) {
...
export const helpers = {
  ...
  printResponse: (response: { [key: string]: any }) =>
    console.log(JSON.stringify(response.json, null, '\t')),

  expect: (value: any, valueName = 'value') =>
    expect(
      value ? valueName : value,
      valueName + ' = ' + JSON.stringify(value, null, 2)
    ),

  expectInResponse: (value: any, response: { [key: string]: any }) =>
  ...
  getUniqueList: (engineRuns: any[], type?: string) => {
  ...
  mediaStreamerHeader: (token: string) => {
    return { headers: { Authorization: 'Bearer ' + token } };
  },
  isLocalHost: (endpoint: string) => {
