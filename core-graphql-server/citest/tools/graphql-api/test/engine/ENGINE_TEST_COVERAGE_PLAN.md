...
| 3 | New: `test/engine/engineWorkflow.spec.ts` | P0 | Drive `engineWorkflow` directly (not only implicitly via `updateEngine`/build actions) through each `EngineWorkflowAction`/`EngineStateAction`; assert out-of-order transitions are rejected (e.g. `deploy` before `ready`, any action after `deleted`); cross-check `validStateActions` against the engine's current `state` at each step; treat post-deploy state as eventually consistent (poll/retry) per the eventing-service dependency noted above |
...
**Suite-level setup**: Org A actor (Developer Editor role, isolated org — same pattern as `engineAuthorization.spec.ts`'s Org A setup) resolves `categoryId`, then `createEngine`
(lands in `draft` per default create behavior) to get engine `E`. Later cases progress `E` through
real states, so ordering matters — run ENTC1–3 first (while `E` is still `draft`), then ENTC7–10
(build/deploy/active/disable), then ENTC4–6 last (delete/undelete), since delete is destructive to
the state under test. Teardown: delete any leftover builds/`E` not already removed by the workflow
itself.
...
| ID | Role | Precondition | Steps | Expected |
|---|---|---|---|---|
| ENTC1 | Org A | `E` exists, `state: draft` | Call `engineWorkflow(input:{id:E, action:enable})` | Rejected — `enable` isn't in `draft`'s `validStateActions` |
| ENTC2 | Org A | `E` in `draft` | Call `engineWorkflow(input:{id:E, action:disable})` | Rejected |
| ENTC3 | Org A | `E` in `draft` | Query `engine(id:E){validStateActions}` | Matches schema's declared `draft`-state actions (e.g. `edit`, `delete`) and excludes `enable`/`disable` |
| ENTC7 | Org A | `E` in `draft`; `createEngineBuild` a build `B` for `E`, then `updateEngineBuild(input:{id:B, action:deploy,...})` | Poll `engine(id:E){state}` on an interval up to a bounded timeout | Eventually becomes `active`; must **not** be `active` immediately after the deploy call returns (async, per `core-eventing-service` dependency) |
| ENTC8 | Org A | `E` confirmed `active` (post-ENTC7) | Query `engine(id:E){validStateActions}` | Includes `disable`/`edit`/`delete`, excludes `enable` |
| ENTC9 | Org A | `E` is `active` | Call `engineWorkflow(input:{id:E, action:enable})` | Rejected — already active, not a valid re-entrant transition |
| ENTC10 | Org A | Call `engineWorkflow(input:{id:E, action:disable})` on `E`, wait for `state: disabled` | Query `engine(id:E){validStateActions}` | Includes `enable`/`edit`/`delete` |
| ENTC4 | Org A | `E` in any non-`deleted` state (use a fresh engine `E2` if `E` was consumed by ENTC7–10) | Call `engineWorkflow(input:{id:E2, action:delete})` | Succeeds, `state` becomes `deleted` |
| ENTC5 | Org A | `E2` in `state: deleted` (post-ENTC4) | Call `engineWorkflow(input:{id:E2, action:edit})`, then `...enable`, then `...disable}` | All rejected — terminal state |
| ENTC6 | Org A | `E2` in `state: deleted` | Call `engineWorkflow(input:{id:E2, action:undelete})` | Restores `E2` to a defined prior/`draft` state (confirm actual target state against docs) |
