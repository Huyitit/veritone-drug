...
flowchart TD
    ...
    subgraph Global Setup [Global Environment Initialization]
        Config --> GlobalSetup[`jest.global.setup.js`]:::setup
        GlobalSetup --> MockServer[Start Ayrshare Mock Server<br/>(if AYRSHARE_MOCK=1)]:::setup
    ...
    subgraph Test Sequencing [`citest-sequencer.js`]
        ...
        PinFirst --> ShardingPhase
        KeepOrder --> ShardingPhase
        ShardingPhase[Shard Tests across processes] --> ColocatePhase[Co-locate specific tests<br/>in the last shard<br/>e.g., orgInviteAppRole]
    end
    ...
    subgraph Global Teardown [Environment Cleanup]
        GlobalTeardownInit --> GlobalTeardown[`jest.global.teardown.js`]:::setup
        GlobalTeardown --> CloseMock[Close Ayrshare Mock Server]:::setup
